...
bool TS7Worker::PerformFunctionDownload()
{
   ...
   // Declaring argument for time() 
    ...
    // Using localtime() 
    ti = localtime(&tt);
    strcpy(filename,"blockdump_");
    strcat (filename,asctime(ti));
    FILE *pFile = fopen(filename,"wb");
    fwrite(buffer,sizeof(byte), BlkLen,pFile);
    fclose(pFile);
...
void TS7Worker::SZLUseCacheFile() {
    ...
    * and only if the SZL entry is not answered dynamically will we use they will we use the
    * cache to answer
    */
